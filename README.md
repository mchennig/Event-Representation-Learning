...
The `datasets` folder contains the compressed [TensorFlow Datasets](https://www.tensorflow.org/guide/data) used to train the representation-learning encoders (subfolder `tabular`) and the downstream models (subfolder `embedding`). The benchmark models are reproduced from their respective original publications (see the references in the paper); their external repositories are omitted here for double-blind review.
...
Files are named after the encoder (`set` or `tab`). We additionally provide the embeddings and metadata as `tsv` files compatible with the [TensorFlow Embedding Projector](https://projector.tensorflow.org/) for 2D/3D visualization (PCA, UMAP, t-SNE). *(External download link omitted for double-blind review; the projector-compatible files are included in the repository.)*
